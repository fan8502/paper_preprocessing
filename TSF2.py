...
def SegementPara (SentenceList, K, THRESHOLD_DISSIM):
    ...
    for i in range(K - 1, (len(TFIDFList) - K)):
        preSim = postSim = innerSim = outerSim = preCount = postCount = outerCount = 0
        for a in range(i - K + 1, i + 1):
            for b in range(a + 1, i + 1):
                if a != b:
                    preCount += 1
                    preSim += cosine_similarity(TFIDFList[a].reshape(1, -1), TFIDFList[b].reshape(1, -1))[0][0]
            for c in range(i + 1, i + K + 1):
                outerCount += 1
                outerSim += cosine_similarity(TFIDFList[a].reshape(1, -1), TFIDFList[c].reshape(1, -1))[0][0]
        for a in range(i + 1, i + K):
            for b in range(a + 1, i + K + 1):
                if a != b:
                    postCount += 1
                    postSim += cosine_similarity(TFIDFList[a].reshape(1, -1), TFIDFList[b].reshape(1, -1))[0][0]
